...
function AuthScreen({ mode, setMode, onAuthenticated }) {
  ...
  return <main className="app"><section className="login"><h1>💬 Poknex</h1><p>{mode === "login" ? "Entre na sua conta para conversar em tempo real." : "Crie sua conta para começar a usar o Poknex."}</p>{error && <div className="status disconnected">🔴 {error}</div>}<form className="login-form" onSubmit={submit}><input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" minLength={3} maxLength={20} autoFocus /><input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Senha" minLength={8} maxLength={128} /><button type="submit" disabled={loading}>{loading ? "Aguarde..." : mode === "login" ? "Entrar" : "Criar conta"}</button></form><button className="auth-switch" type="button" onClick={() => { setMode(mode === "login" ? "register" : "login"); setError(""); }}>{mode === "login" ? "Ainda não tenho uma conta" : "Já tenho uma conta"}</button></section></main>;
}
...
export default function AppEdit() {
  ...
  useEffect(() => { queueRef.current = offlineQueue; localStorage.setItem(QUEUE_KEY, JSON.stringify(offlineQueue)); }, [offlineQueue]);
  useEffect(() => { messagesRef.current = messages; localStorage.setItem(STORAGE_KEY, JSON.stringify(messages)); }, [messages]);
  useEffect(() => { userRef.current = user; }, [user]);
  useEffect(() => { if (connectionStatus !== "reconnecting") return; const id = setInterval(() => setReconnectSeconds((value) => value > 1 ? value - 1 : 10), 1000); return () => clearInterval(id); }, [connectionStatus]);

  useEffect(() => {
    const ids = new Set();
    for (const item of messages) if (item?.userId && !profilesById[item.userId] && !profileFetchesRef.current.has(item.userId)) ids.add(item.userId);
    for (const item of users) if (item?.id && item.avatar === "" && !profilesById[item.id] && !profileFetchesRef.current.has(item.id)) ids.add(item.id);
    for (const id of ids) {
      profileFetchesRef.current.add(id);
      getPublicProfile(id).then((remote) => {
      ...
      }).catch(() => {}).finally(() => profileFetchesRef.current.delete(id));
    }
  }, [messages, users, profilesById]);

  function syncProfile(nextUser) {
    setUser(nextUser); setProfile(nextUser); userRef.current = nextUser;
  ...
  function mergeUser(incoming) {
    ...
    setUsers((current) => { const index = current.findIndex((item) => String(item.id) === String(incoming.id)); if (index < 0) return [...current, incoming]; const next = [...current]; next[index] = { ...next[index], ...incoming }; return next; });
  }
  function flushQueue() {
  ...
  function connect(token) {
    ...
    socketRef.current?.close(); sessionRef.current = true; setConnected(false); setConnectionStatus("connecting");
    const socket = createWebSocket(token, {
      onOpen() { if (generation !== generationRef.current) return; setConnected(true); setConnectionStatus("connected"); setReconnectAttempt(0); setReconnectSeconds(0); setTimeout(flushQueue, 0); },
      onMessage(data) {
        if (generation !== generationRef.current) return;
        if (data?.type === "users") { const list = Array.isArray(data.users) ? data.users : []; setUsers(list); list.forEach(mergeUser); return; }
      ...
      onClose() { if (generation !== generationRef.current) return; setConnected(false); setConnectionStatus(sessionRef.current ? "reconnecting" : "disconnected"); if (sessionRef.current) setReconnectSeconds(10); },
      onReconnecting(_delay, attempt) { if (generation !== generationRef.current || !sessionRef.current) return; setConnected(false); setConnectionStatus("reconnecting"); setReconnectAttempt(attempt); setReconnectSeconds(10); },
  ...
  useEffect(() => {
    ...
    async function restore() {
      ...
      try { const currentUser = await me(); if (cancelled) return; syncProfile(currentUser); sessionRef.current = true; setAuthChecked(true); connect(token); }
      catch { if (!cancelled) { clearToken(); sessionRef.current = false; setAuthChecked(true); setConnectionStatus("disconnected"); } }
    }
    restore();
    return () => { cancelled = true; sessionRef.current = false; ++generationRef.current; socketRef.current?.close(); };
  }, []);

  function handleAuthenticated(currentUser) { syncProfile(currentUser); sessionRef.current = true; setAuthChecked(true); connect(getToken()); }
  function sendMessage(event) {
    event.preventDefault(); const text = messageInput.trim(); if (!text) return;
    const id = makeId(); const sender = userRef.current;
    const optimistic = { type: "message", messageId: id, userId: sender?.id, username: sender?.username, displayName: sender?.displayName, avatar: sender?.avatar || "", status: sender?.status || "", message: text, timestamp: Date.now(), offline: !connected, deliveryStatus: connected ? "sending" : "pending" };
    if (connected && socketRef.current?.sendMessage(text, id)) { setMessages((current) => [...current, optimistic]); setMessageInput(""); return; }
    setOfflineQueue((current) => [...current, { id, message: text, createdAt: Date.now(), userId: sender?.id, username: sender?.username, displayName: sender?.displayName, avatar: sender?.avatar || "" }]); setMessages((current) => [...current, optimistic]); setMessageInput("");
  }
  function beginEdit(message) { setContextMenu(null); setEditError(""); setEditingId(message.messageId); setEditingText(message.message); }
  function cancelEdit() { if (!editSaving) { setEditingId(null); setEditingText(""); setEditError(""); } }
  function saveEdit(event) {
    event.preventDefault(); const text = editingText.trim(); if (!editingId || !text) { setEditError("A mensagem não pode ficar vazia."); return; }
    if (!connected || !socketRef.current?.sendEditMessage(editingId, text)) { setEditError("Aguardando conexão para editar."); return; }
  ...
  async function saveProfile(event) {
    event.preventDefault(); setProfileError(""); setProfileSaving(true); const form = new FormData(event.currentTarget); const oldUsername = user.username;
    try { let nextAvatar = profile?.avatar || ""; if (avatarFileRef.current) nextAvatar = await uploadAvatar(avatarFileRef.current); const updated = await updateProfile({ username: String(form.get("username") || oldUsername).trim(), displayName: String(form.get("displayName") || oldUsername).trim() || oldUsername, avatar: nextAvatar, status: String(form.get("status") || "").trim() }); avatarFileRef.current = null; syncProfile(updated); setProfileOpen(false); if (updated.username !== oldUsername) connect(getToken()); }
    catch (error) { setProfileError(error.message || "Não foi possível atualizar o perfil."); }
    finally { setProfileSaving(false); }
  }
  function chooseAvatar(event) {
    const file = event.target.files?.[0]; if (!file) return; if (!file.type.startsWith("image/")) { event.target.value = ""; return; }
    if (file.size > 2 * 1024 * 1024) { alert("Escolha uma imagem de até 2 MB."); event.target.value = ""; return; }
    avatarFileRef.current = file; const reader = new FileReader(); reader.onload = () => setProfile((current) => ({ ...(current || userRef.current), avatar: String(reader.result) })); reader.readAsDataURL(file); event.target.value = "";
  }
  useEffect(() => () => { clearTimeout(longPressRef.current); }, []);
  useEffect(() => { function click() { setContextMenu(null); } if (contextMenu) { document.addEventListener("click", click); return () => document.removeEventListener("click", click); } }, [contextMenu]);
  ...
  return <main className="app"><section className="chat">
    <aside className="sidebar">
      <div className="sidebar-header"><div className="profile-summary" onClick={() => { setProfileError(""); setProfileOpen(true); }}><div className="avatar profile-avatar">{avatar ? <img src={avatar} alt="" /> : displayName.slice(0, 1).toUpperCase()}</div><div><h2>{displayName}</h2><p>@{user.username}</p><small>{profile?.status || "Sem status"}</small></div></div></div>
      <div className="users-title">Usuários online — {users.length}</div>
      <ul className="users">{users.map((onlineUser) => <li className="user" key={onlineUser.id}><div className="avatar user-avatar">{onlineUser.avatar ? <img src={onlineUser.avatar} alt="" /> : userInitial(onlineUser)}</div><div className="user-info"><strong>{onlineUser.displayName || onlineUser.username}</strong><span><span className="online-dot" />@{onlineUser.username}</span></div></li>)}</ul>
    ...
    <div className="chat-content">
      ...
      <form className="message-form" onSubmit={sendMessage}><textarea placeholder={connected ? "Digite uma mensagem..." : "Digite uma mensagem offline..."} value={messageInput} onChange={(e) => setMessageInput(e.target.value)} rows={1} maxLength={1000} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(e); } }} /><button type="submit" disabled={!messageInput.trim()}>Enviar</button></form>
