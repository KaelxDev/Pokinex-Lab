...
function App() {
  ...
  useEffect(() => {
    ...
    for (const userId of profileIds) {
      if (profileFetchesRef.current.has(userId)) continue;
      profileFetchesRef.current.add(userId);
      getPublicProfile(userId)
        .then((remoteUser) => {
          setProfilesById((current) => ({ ...current, [remoteUser.id]: remoteUser }));
          setUsers((current) => current.map((item) => item.id === remoteUser.id ? { ...item, ...remoteUser } : item));
          setMessages((current) => current.map((item) => item.userId === remoteUser.id ? { ...item, username: remoteUser.username, displayName: remoteUser.displayName, avatar: remoteUser.avatar, status: remoteUser.status } : item));
        })
        .catch(() => {})
        .finally(() => profileFetchesRef.current.delete(userId));
    }
  ...
  function mergeOnlineUser(incomingUser) {
    ...
    setProfilesById((current) => ({ ...current, [incomingUser.id]: { ...current[incomingUser.id], ...incomingUser } }));
    setUsers((current) => {
      ...
      onMessage(data) {
        ...
        if (data?.type === "users") {
          ...
          setProfilesById((current) => {
            const next = { ...current };
            for (const onlineUser of nextUsers) if (onlineUser?.id) next[onlineUser.id] = { ...next[onlineUser.id], ...onlineUser };
            return next;
          });
          return;
        }
        ...
        if (data?.type === "profile_updated" && data.user) {
          ...
          if (userRef.current?.id === data.user.id) syncProfile({ ...userRef.current, ...data.user });
  ...
  return <main className="app"><section className="chat">
    <aside className="sidebar">
      <div className="sidebar-header">
        <div className="profile-summary" onClick={() => { setProfileError(""); setProfileOpen(true); }}>
          <div className="avatar profile-avatar">{avatar ? <img src={avatar} alt="Avatar" /> : displayName.slice(0, 1).toUpperCase()}</div>
      ...
      <ul className="users">
        {users.map((onlineUser) => <li className="user" key={onlineUser.id}>
          <div className="avatar user-avatar">{onlineUser.avatar ? <img src={onlineUser.avatar} alt="Avatar" /> : userInitial(onlineUser)}</div>
    ...
    <div className="chat-content">
      <header className="chat-header">
        <div><h1># geral</h1>{connectionStatus === "reconnecting" ? <div className="connection connecting">🟡 Reconectando... tentativa #{reconnectAttempt || 1} • próxima tentativa em {reconnectSeconds || 10}s</div> : connectionStatus === "connecting" ? <div className="connection connecting">🟡 Conectando...</div> : <div className="connection"><span className="online-dot" />Online</div>}</div>
      ...
      <form className="message-form" onSubmit={sendMessage}>
        <textarea placeholder={connected ? "Digite uma mensagem..." : "Digite uma mensagem offline..."} value={messageInput} onChange={(event) => setMessageInput(event.target.value)} onKeyDown={handleMessageKeyDown} rows={1} maxLength={1000} />
    ...
    {profileOpen && <div className="profile-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}>
      <form className="profile-modal" onSubmit={saveProfile}>
        ...
        <div className="profile-preview">
          <div className="avatar profile-avatar profile-preview-avatar">{avatar ? <img src={avatar} alt="Avatar" /> : displayName.slice(0, 1).toUpperCase()}</div>
