...
function App() {
  ...
  function connect(nameOverride = username, isRestore = false) {
    ...
    const nextProfile = profile || makeProfile(name); nextProfile.username = name; setProfile(nextProfile);
  ...
  function saveProfile(event) { event.preventDefault(); const form = new FormData(event.currentTarget); const next = { username, displayName: String(form.get("displayName") || username).trim() || username, avatar: String(form.get("avatar") || "").trim(), status: String(form.get("status") || "").trim() }; setProfile(next); setProfileOpen(false); }
