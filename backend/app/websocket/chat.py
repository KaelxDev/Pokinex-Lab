...
class ConnectionManager:
    ...
    async def send_message(self, user: dict, message: str, message_id: str | None = None, sender: WebSocket | None = None):
        user = self.get_user(sender) if sender else None
        user = user or self.active_connections.get(sender) if sender else user
        ...
        self.sequence += 1
        await self.broadcast({"type":"message","messageId":message_id,"userId":user["id"],"username":user["username"],"displayName":user["displayName"],"avatar":user["avatar"],"status":user["status"],"message":message,"timestamp":self.get_timestamp(),"sequence":self.sequence})
    ...
    async def edit_message(self, user: dict, message_id: str, message: str, sender: WebSocket):
        ...
        if owner_id is None:
            await sender.send_json({"type":"error","action":"edit_message","messageId":message_id,"message":"Mensagem não encontrada."}); return
        if owner_id != user["id"]:
        ...
        await self.broadcast({"type":"message_edited","messageId":message_id,"userId":user["id"],"username":user["username"],"displayName":user["displayName"],"avatar":user["avatar"],"status":user["status"],"message":message,"editedAt":self.get_timestamp(),"edited":True})
