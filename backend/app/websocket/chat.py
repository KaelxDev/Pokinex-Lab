...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user: dict):
        await websocket.accept(); self.active_connections[websocket] = user
        await self.broadcast({"type":"system","event":"user_joined","userId":user["id"],"username":user["username"],"displayName":user["displayName"],"avatar":user["avatar"],"message":f"{user['username']} entrou no chat.","timestamp":self.get_timestamp()}); await self.send_users()
    ...
    def update_user(self, user: dict) -> bool:
        ...
        for ws,current in list(self.active_connections.items()):
            if current["id"]==user["id"]: self.active_connections[ws]=user; updated=True
        return updated
    def get_user(self, websocket: WebSocket): return self.active_connections.get(websocket)
    async def broadcast(self, data: dict):
        disconnected=[]
        for ws in list(self.active_connections):
            try: await ws.send_json(data)
            except Exception: disconnected.append(ws)
        for ws in disconnected: self.active_connections.pop(ws,None)
    ...
    async def broadcast_profile_update(self,user:dict):
        await self.broadcast({"type":"profile_updated","user":{"id":user["id"],"username":user["username"],"displayName":user["displayName"],"avatar":user["avatar"],"status":user["status"]},"timestamp":self.get_timestamp()}); await self.send_users()
    async def send_message(self,user:dict,message:str,message_id:str|None=None,sender:WebSocket|None=None,reply_to_message_id:str|None=None):
        ...
        if reply_to_message_id:
            ...
            if original:
                reply_to={"messageId":original["message_id"],"userId":original["user_id"],"username":original["username"],"displayName":original["display_name"],"avatar":original["avatar"],"message":"Esta mensagem foi excluída" if original["deleted_at"] else original["message"],"deleted":bool(original["deleted_at"])}
        timestamp=self.get_timestamp()
        if message_id:
            self.processed_message_ids.add(message_id); self.message_owners[message_id]=user["id"]; save_message(message_id,user["id"],message,timestamp,reply_to_message_id)
        self.sequence+=1
    ...
    async def edit_message(self,user:dict,message_id:str,message:str,sender:WebSocket):
        ...
        edited_at=self.get_timestamp(); update_message(message_id,user["id"],message,edited_at); self.sequence+=1
        event={"messageId":message_id,"userId":user["id"],"username":user["username"],"displayName":user["displayName"],"avatar":user["avatar"],"status":user["status"],"message":message,"editedAt":edited_at,"edited":True,"sequence":self.sequence}; await self.broadcast({"type":"message_edited",**event}); await self.broadcast({"type":"message",**event}); await sender.send_json({"type":"edit_ack","messageId":message_id})
