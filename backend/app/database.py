...
def initialize_database() -> None:
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS messages (message_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, message TEXT NOT NULL, created_at TEXT NOT NULL, edited_at TEXT, deleted_at TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);
        CREATE INDEX IF NOT EXISTS idx_messages_user_id ON messages(user_id);
        """)
        try: connection.execute("ALTER TABLE messages ADD COLUMN deleted_at TEXT")
...
def save_message(message_id: str, user_id: int, message: str, created_at: str) -> None:
    connection = get_connection()
    try:
        connection.execute("INSERT OR IGNORE INTO messages (message_id, user_id, message, created_at) VALUES (?, ?, ?, ?)", (message_id, user_id, message, created_at))
        connection.commit()
    finally: connection.close()
